    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm">Powered by <span className="font-mono text-chzzk">yt-dlp</span> wrap engine</p>
        </div>
      </div>
      ...
      <div className="bg-chzzk-card p-6 rounded-xl border border-gray-800 shadow-lg">
        <form onSubmit={handleAnalyze} className="flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Paste VOD or Clip URL here (e.g., https://chzzk.naver.com/video/...)"
            />
          </div>
          <button
            type="submit"
            disabled={isAnalyzing || !url}
            className={`w-full sm:w-auto px-6 py-3 rounded-lg font-bold flex items-center justify-center space-x-2 transition-all ${
              isAnalyzing 
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
          <span className="flex items-center"><CheckCircle className="w-3 h-3 mr-1 text-green-500" /> Supports 1080p60</span>
          <span className="flex items-center"><CheckCircle className="w-3 h-3 mr-1 text-green-500" /> Auto-remux to MP4</span>
          <span className="flex items-center"><CheckCircle className="w-3 h-3 mr-1 text-green-500" /> Adult Verification Cookie Supported</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        {tasks.length === 0 ? (
          <div className="text-center py-20 bg-chzzk-card/30 rounded-xl border border-gray-800 border-dashed">
             ...
             <p className="text-sm text-gray-600">Paste a link above to start downloading</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {tasks.map((task) => (
              <div key={task.id} className="bg-chzzk-card border border-gray-800 p-4 rounded-xl flex flex-col sm:flex-row items-center gap-4 hover:border-gray-700 transition-colors">
                <div className="w-full sm:w-32 h-40 sm:h-20 bg-gray-900 rounded-lg overflow-hidden flex-shrink-0 relative">
                ...
                <div className="flex w-full sm:w-auto sm:flex-col gap-2">
                   <button className="flex-1 sm:flex-none flex items-center justify-center p-2 hover:bg-gray-800 rounded-lg text-gray-400 hover:text-white transition-colors bg-gray-800/50 sm:bg-transparent">
